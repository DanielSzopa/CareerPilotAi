...
#### 3.4.1 EmailService Tests

**File:** `tests/CareerPilotAi.IntegrationTests/Services/EmailServiceTests.cs`

**Test Cases:**

1. **Send Emails**
   - Valid email and link → Should not throw
   - SendGrid called → With correct template
   - Template variables → Should be set

2. **Error Scenarios**
   - Null email/link → Should throw ArgumentNullException
   - SendGrid error → Should throw with error message
   - Missing configuration → Should throw InvalidOperationException

#### 3.4.2 TimeZoneService Tests

**File:** `tests/CareerPilotAi.IntegrationTests/Services/TimeZoneServiceTests.cs`

**Test Cases:**

1. **Get All Timezones**
   - Retrieve all → Should return all in DB
   - Sorted → Should be ordered by Name

2. **Check Existence**
   - Valid timezone → Should return true
   - Invalid timezone → Should return false

#### 3.4.3 OpenRouter Service Tests

**File:** `tests/CareerPilotAi.IntegrationTests/Services/OpenRouterServiceTests.cs`

**Test Cases:**

1. **Parse Job Description**
   - Valid description → Should return parsed model
   - Response parsing → Should deserialize correctly

2. **Generate Interview Questions**
   - Valid input → Should return questions
   - Multiple questions → Should be in list

3. **Error Handling**
   - HTTP error → Should throw
